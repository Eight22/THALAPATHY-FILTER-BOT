...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"<b>ʜᴇʏ {user} 😍 ,\n\nYou Cant't Get Movies Form Here. Request It In Our <a href=https://t.me/UncutFile>Movies Group</a> Or Click Here Button Below 👇</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 Request Here ", url=f"https://t.me/Uncutfiles")]])
    )
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"🙄 Hello{query.from_user.first_name},\nThis Is Not Your Serach Result,\nSearch Your'ꜱ...",
                show_alert=True,
            )
    except:
        pass
    ...
    offset = 0
    btn.append([InlineKeyboardButton(text="🏄 Back To File", callback_data=f"fl#homepage#{key}")])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if groupids is None:
            await query.message.edit_text(
                "♻️ There Are No Active Connection!! 🔗 Connect To Same Groups First.",
            )
            return await query.answer(MSG_ALRT)
        buttons = []
        ...
        if buttons:
            await query.message.edit_text(
                "♻️ Your Connected Group Details ;\n\n",
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("⚡ Join Our Channel 🚀", show_alert=True)
            return
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton('Short Link',
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    InlineKeyboardButton('Protect Contant',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
                    InlineKeyboardButton('Yes' if settings["file_secure"] else 'No',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],
        ...
        if query.from_user.id in ADMINS:
            ...
            await query.answer("You Dont't Have Sufficiant Right To Do This !", show_alert=True)
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🌐 Global', callback_data='global_filters')
        ],[
